...
const getCachePath = async (repository) => {
  ...
  // 判断是否已经缓存了最新的模板文件
  if (fs.existsSync(repositoryPath) && cacheJson[repository.name] && cacheJson[repository.name].updatedAt === repository.updatedAt) {
    return repositoryPath
  } else {
    await Util.downloadGitRepo(repository.resourcePath.slice(1), repositoryPath)
